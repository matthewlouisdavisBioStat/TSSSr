...
#' @param degree = 3 what is the maximum polynomial degree for numeric and integer parameters? First order interactions will be included regardless. Goes from 3 to and including 9.
#' @param plot_predicted_performance should the predicted performance of unevaluated grid points be plotted?
#' @param max_grid_rows what is the maximum design-matrix number of rows?
#' @param max_knots what is the maximum number of knots to include (for fitting splines)?
#' @param obs_per_spline_coefficient function that takes as a single argument the number of knots, and returns the number of observations required to add that knot
#' @param random  when algorithm fails, should all grid points or a single randomly sampled one (default) be evaluated?
#' @param use_transf  should a default variance-stabilizing transformation be used?
#' @param log_outcome  should the metric being modeled be log-transformed as an outcome?
#' @param logit_outcome should the metric being modeled be logit-transformed as an outcome?
#' @param save_model_fit should the design matrix and beta coefficients be saved to an RData file? 
#' @param include_numeric_interactions should numeric-numeric interactions be included?
#' @param include_high_order_interactions should degree 3-and-higher numeric polynomial terms and categorical interactions be included?
#' @param include_quadratic_interactions  should degree 2 numeric polynomial terms and categorical interactions be included?
#' @param qualify_numeric_numeric_interactions should three-way interactions between categorical and first-order numeric pairings be included?
#' @param save_model_fit_lable  label for the data file saving the beta coefficients and design matrix,
#' @param generic_tolerance tolerance for convergence on a variety of tasks
#' @param cardinal_knots should the knots be spaced evenly?
#' @param latin_grid should numerics (not integers) be latin-grid'd?
#' @param enforce_positive_definiteness should we ensure that each polynomial partition unconstrained is full rank?
#' @param print_colnames  should the column names of the design matrix be printed?
#' @param acquisition_function custom acquisition function
#' @param stopping_rule_total stopping rule will be considered after this many grid points have been evaluated since improvement
#'
#' @details A custom set_optim_method to be paired with MachineShop for tuning models and inputs using Thompson Sampling with Smoothing Splines.
#'
#' @return A model specification object ready to be fit, equipped with Thompson Sampling with Smoothing Splines.
#'
#' @importFrom foreach foreach
#'
#' @seealso \code{\link[MachineShop]{set_optim_method}}, \code{\link[MachineShop]{ModelSpecification}}
#'
#'
#' @examples
#' \donttest{
...
################################################################################################
## ## ## ## Thompson Sampling with Smoothing Splines for Hyperparameter Optimization ## ## ## ## 
  ...
  ## save base R optim function
  optimStats <- function(...){
    stats::optim(...)
  }
  
  ## probability of NOT being greedy, initialized
  ## (choosing the grid point with best predicted performance deterministically)
  prob_random <- 1
  ...
  ## initialize box-cox transform penalty, if distance metric will be used
  pwr_lmbda_opt <- NULL
 
  ## use logit/log transform if requested
  if(!is.null(log_outcome)){
    if(log_outcome) use_transf <- F
  }
  if(!is.null(logit_outcome)){
    if(logit_outcome) use_transf <- F
  }
  
  ## check if degree is less than 9
  if(!(degree >= 3 & degree <= 9)){
    stop("Polynomial degree must be between 3 and 9")
  }
  ...
  ## latin grid continous hyperparameters
  if(latin_grid){
  ...
  cats <- which((sapply(og_grid[1, ], class) %in% 
                   c("factor", "character")) & 
                  unq_g_1)
  ...
  make_design_matrix <- function(grid,drop_cols = T, 
    ...
    if (!is.null(interactions)) {
      colnames(interactions) <-
        ## interactions get penalized equal to quadratic terms for two way, cubic terms for three way
  ...
  ## scale so all variables have equal sd, except cat's which are kept as indicators
  ...
  ## print design matrix column names, if desired
  if(print_colnames){
    print(cbind(colnames(design_mat)))
  }
  
  ## for how many grid points are needed we need before adding a new spline
  p0 <- ncol(design_mat)
  ...
  ## wrap around the optim function starting.....now
  ## follows from MachineShop ?set_optim_method
  set_optim_method(
    object,
    fun = function(optim, grid, ...) {
      ...
      if(initial_iterations >= nrow(grid)){
        ...
        ## for each grid point randomly sampled, evaluated and extract metric
        count <- 1
        ...
        ## keeping track of the sign of transformation - we negate our "y" for metrics to be minimized
        ...
        if(!use_transf){
          ...
          if(log_outcome){
            y <- transf_sign*log(transf_sign*y + generic_tolerance)
          }
          if(is.null(logit_outcome)){
            logit_outcome <- F
          }
          if(logit_outcome){
            y <- transf_sign*(log(transf_sign*y+generic_tolerance) - 
                              log(1-transf_sign*y-generic_tolerance))
          }
        } else {
          
          ## if metrics are distance type, optimize the box-cox penalty
          ## using the -log pvalue of a Shapiro-Wilks test
          ## applied to the transformed performance
          if(metrics %in% distance_type_metrics){
            ...
            if(length(unique(y)) > 1){
              pwr_lmbda_opt <- try({optimize(
                f = function(l){
                  z <- (transf(y[which],'distance',pwr_lmbda = l))
          ...
          ## otherwise, if proportion type, apply the arcsin-square root
          } else if(metrics %in% proportion_type_metrics){
            y <- transf_sign*y
            y <- transf_sign*transf(y,'proportion')

          ## otherwise, if interval type, apply the arc hyperbolic tangent
          }  else if(metrics %in% interval_type_metrics){
            y <- transf_sign*y
            y <- transf_sign*transf(y,'interval')

          ## finally, maybe don't apply any transformation at all
          } else {
            y <- transf_sign*y
          }
	  ...
	  ## center and scale outcome for computationally stability when finding
        ## meta-hyperparameters for Lambda 
	  ...
	  ## remove nas from consideration
        ...
        ## L.O.O. cross-Validation for choosing idge Penalty And Slope Fxn
        tXy <- t(X) %*% Y
        ...
        ## when errors occur, plug in the geneic tolerance for penalties,
        ## and gamma = 1 for the slope
        if(any(class(opt_ridge) == 'try-error')){
          opt_ridge <- c(generic_tolerance,1,generic_tolerance)
        }
        
	  ## adjust ridge penalty, matrix components
	  ...
	  ## ## Sampling Time ## ## 
	    ...
	    ## deterministic a posteriori expected best
        ...
        if(length(preds) == 1){
          ...
          ## initialize some terms to be updated iteratively
          steps <- 0
		  ...
		  ## the actual sample-new-grid point step
              ...
              ## if this is the last iteration and we're optimizing using L-BFGS-B, we need to prepare for interpolation
              if(steps == (times - 1)){
                ## use L-BFGS-B for optimizing over numerical predictors
                if(length(c(numerics,integers)) >= 1){
			## get min value above optimal thus far, and max value below, each numeric tuning parameter
			...
			## optimize between these bounds for best-performing hyperparameter values
			...
			## code up a new grid point to evaluate
            ...
            if(plot_predicted_performance){
              ...
              if(use_transf){
                if(metrics %in% distance_type_metrics){
                  plot_preds <- transf_sign*un_transf(transf_sign*plot_preds,'distance',pwr_lmbda = pwr_lmbda_opt)
                } else if(metrics %in% proportion_type_metrics){
                  plot_preds <- transf_sign*un_transf(transf_sign*plot_preds,'proportion')
                } else if(metrics %in% interval_type_metrics){
                  plot_preds <- transf_sign*un_transf(transf_sign*plot_preds,'interval')
                }
              }
            ...
            ## update best-so-far and evaluate new grid point selected
            for(gp in as.numeric(new_grid_point)){
              ...
              if(!use_transf){
                if(log_outcome){
                  y[paste0(gp)] <- transf_sign*log(transf_sign*opt+generic_tolerance)
                } else if(logit_outcome){
                  y[paste0(gp)] <- transf_sign*log(transf_sign*opt+generic_tolerance) - 
                    transf_sign*log(1-transf_sign*opt-generic_tolerance)
                } else {
            ...
            ## center and scale for computational stability, tuning meta-hyperparameters
            y <- cs[[1]]
            mu <- cs[[2]]
            sig <- cs[[3]]
            
		## extract non-NA inds to model with
            inds_to_rem <- which(is.na(y))
            if(length(inds_to_rem) > 0){
              nas <- as.numeric(names(y[inds_to_rem]))
            } else{
            ...
            ## add new grid point to your collection
            grid_points <- unique(c(grid_points,new_grid_point))
            
            
            ## re-fit the model again
            X <- design_mat[grid_points[!(grid_points %in% nas)],]
            Y <- y[!(as.numeric(names(y)) %in% nas)]
            max_transf_Y <- max(Y)
            if(!is.na(y[paste0(new_grid_point)])){
              if(num_knots == 0){
                ...
                ## smoothing penalty matrix, update other matrix components
                knot_ridge <- diag(
                  sapply(colnames(XX_knot),function(xx){
                    if(adjust[xx]){
                      (opt_ridge_knot[1])^
                ...
                mu_new <- G_knot %*% (tXy)
                sigmaSq_knot <- mean((Y - X_knot %*% mu_new)^2)
                Amat2 <- G_knot[,1:2]
                KC <- 0*Amat2
                tGAmat2 <- t(G_knot) %*% KC
                ...
                ## compute jackknife/leave-one-out residuals
                VU <- G_knot
                denom <- 1 - diag(X_knot %*% VU %*% t(X_knot))
                YminusYhat <- Y-X_knot %*% mu_new
                ...
                ## should we trigger the addition of more knots?
                ...
                ## make the smoothing penalty prior, if you haven't already
                if(!made_R) {
                ...
                ## expand the cubic expansions into cubic spline expansions
                X_knot <- knot_expand2(X,
                                       Q[grid_points[!(grid_points %in% nas)],])
                
		    ## ## Enforcing Positive-Definiteness for Bonafide Guaranteed Multimodal Exploration ## ## 
                ...
                ## make a knot constraint (KC) constraint matrix  (serves the function of A)
                ...
                ## matrix components and finding new smoothing penalty/sloped-ridge penalty
                XX_knot <- t(X_knot) %*% X_knot
                Amat2 <- cbind(KC)
                tXy <- t(X_knot) %*% Y
                opt_ridge_knot <- try({optimStats(
		    ...
		    ## update sloped-ridge penalty
                ...
                ## Use method of Lagrangian Mults. to find posterior mean under constraint
                tGAmat2 <- cbind(G_knot %*% Amat2)
                mu_new <- G_knot %*% (tXy)
		    ...
		    ## construct U
                ...
                ## check for nas, replace mu_new with 0 in this case
                if(any(na.omit(c(is.na(mu_new),
                                 is.nan(mu_new),
                                 !is.finite(mu_new))))){
                  mu_new <- cbind(rep(0,length(c(mu_new))))
                  sigmaSq_knot <- 0
                } else {

			## new fitted residuals
                  new_resids <- Y - 
                    X_knot %*% 
                    mu_new
                  ...
                  if((nrow(X_knot) > obs_per_spline_coefficient(num_knots+1)) & 
            ...
            ## get new expectations for posterior-predictive distributions
            ...
            if(!is.finite(sigmaSq_knot) | (sigmaSq_knot <=0) | is.nan(sigmaSq_knot) | is.na(sigmaSq_knot)){
              ...
              ## compute matrix/scalar components needed for updating posteriors
              svd <-  svd(G_knot)
              n <- nrow(X_knot)
              r <- qr(XX_knot)$rank
              q <- ncol(KC)
		  ...
		  ## the sigma^2 that was drawn
              sigSq <- ifelse(sum(abs(mu_new)) > 0,
                                   my_rinvgamma(1, 
                                                -1 + max(n - r,3)/2, 
                                                0 + reml/2,
                                                jackknife_sigSq_est),
		  ...
		  ## the beta that was drawn under constraint
		  ...
		  ## predictions conditional upon beta that was drawn
              thompson_preds <- 
                oog_expand %*% 
                thompson_sample_knot
              
              ## weights are the probability any candidate grid point is selected next
              if(sample_random){
                weights <- sample(c(1,rep(0,length(thompson_preds)-1)))
              } else if(!is.null(acquisition_function)){

		    ## use custom acquisition function
                weights <- acquisition_function(thompson_preds,
                                                sqrt(sigSq),
                                                max(Y),
                                                steps,
		  ...
		  ## code for triggering knot addition
              if(!cardinal_knots){
		...
		## finished iteration, start again
            steps <- steps + 1
          }

          ## if desired, save the fitted coefficients, the design matrix, and the indices of grid points evaluated
          if(save_model_fit){
            save(mu_new,design_mat,X_knot,grid_points,pwr_lmbda_opt,file = save_model_fit_lable)
          }
        } else {
          NULL
        }
